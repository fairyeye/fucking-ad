...
欢迎参与维护 **fucking-ad (流氓弹窗广告恶行录)**！

互联网广告乱象之所以屡禁不止，正是因为单个消费者的愤怒容易被时间冲淡。通过集体的力量记录每一次骚扰，我们能让流氓营销付出真正的商业代价。

---

## 🛡️ 基本准则 (Code of Conduct)

为了保证项目的公信力、严肃性和法律合规性，所有提交必须遵守以下原则：

1. **真实客观，铁证如山**：
   - **必须** 提供真实未篡改的屏幕截图、录屏或受害证据。
   - 严禁出于商业竞争恶意捏造、抹黑或无中生有。没有事实凭据的条目将不予收录或直接驳回。
2. **就事论事，理性记录**：
   - 记录聚焦于「具体的广告骚扰、诱导点击、滥用传感器」等客观行为，不进行人身攻击。
3. **改过自新，鼓励良性循环**：
   - 若某厂商主动彻底整改了流氓广告行为（例如彻底移除了摇一摇跳转并向公众道歉），可在验证属实后更新状态并下调抵制评级。

---

## 方式一：通过 Issue 举报（推荐普通用户）

如果你不想摆弄代码，只需 1 分钟填表：
1. 点击访问项目的 [New Issue](../../issues/new/choose)；
2. 选择 **「📢 举报流氓弹窗广告」**；
3. 按照模板逐项填写广告品牌、受害宿主APP、上传截图；
4. 提交后，维护团队会复核并自动转录为数据入库。
...
advertiser:
  ...
  boycott_level: 5                     # 建议抵制程度 (1:轻度骚扰, 3:严重流氓, 5:坚决终身不买)
host_app:
  name: "酷狗音乐"                      # 哪个宿主 APP 弹出的广告
  platform: "iOS"                      # iOS / Android / HarmonyOS / Windows / macOS / Web
  version: "12.0.1"                    # 版本号（选填）
offense_type:                          # 作恶类型，可多选
  - "摇一摇跳转"
  - "开屏广告/全屏遮罩"
description: >-
  走路时稍微晃动手机，立即强行唤醒该购物软件；返回原APP后发现关闭按钮延迟3秒才显示，属于典型流氓诱导。
evidence:
  images:
